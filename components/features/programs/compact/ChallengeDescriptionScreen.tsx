...
type ChallengeDescriptionScreenProps = {
  workout: WorkoutType;
...
const ChallengeDescriptionScreen: React.FC<ChallengeDescriptionScreenProps> = ({
  workout,
  challenge,
  back,
  skip = () => {},
  ...
  useEffect(() => {
    if (challenge.isBonus && challenge.bonusDescription) {
      const descrArray = localization.t(challenge.bonusDescription).split("{0}");
      const node = descrArray.map((txt: string, index: number) => {
        return (
      ...
      setDescrText(<Text style={styles.listSentence}>{node}</Text>);
    } else if (challenge.isTimer) {
      setDescrText(
  ...
  return (
    <View style={[styles.container, !isCompact && { paddingHorizontal: 32, paddingTop: 32 }]}>
      <WorkoutNavHeader text={localization.t(workout.title)} back={back} />
      <ScrollView
